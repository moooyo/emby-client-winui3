...
## Poster decoder baseline

`poster-decoder-baseline.json` and `poster-input.png` preserve an independent Image-only NativeAOT observation using the linked product `NativePosterDecoder` callback helper. The exact input was downloaded once from the identity-checked 5,000-item synthetic fixture, then authentication and HTTP were closed before measurement. All eighty cycles used the same 8,404-byte, 480 by 720 PNG; scale 1.5 selected decode dimensions 264 by 396. Every cycle bound inside the helper callback, observed two render callbacks, cleared the source, and observed two further callbacks. This is render-boundary evidence, not captured pixels.

The terminal result is `BaselineCompleted`, not library acceptance. Baseline handles were 905, cycles 20-80 remained at 919, and final two/ten/thirty-second idle handles were 919/915/915. Late-ten versus cycles 11-20 median handle growth and last-forty handle slope were both zero. Private bytes rose from 89,989,120 to 97,239,040 at cycle 80 and 97,452,032 after final idle; managed bytes rose from 1,305,328 to 6,625,024. All GC collection counts stayed zero. The harness also allocates cumulative serialized reports, so this memory change is not isolated bitmap retention evidence. No forced GC, explicit WinRT async-operation Close, or bitmap reuse was added.

This single-Image baseline does not reproduce the large library's handle climb. It does not cover many concurrently realized cards, navigation cancellation, image caches, network activity during decoding, or long-duration behavior. It provides no basis by itself to alter `StoreAsync`/`SetSourceAsync` cleanup. All per-cycle and idle samples, actual source/dependency hashes, the helper hash, and the NativeAOT executable SHA-256 are retained in the receipt. The executable hash is `2e0c20295f8bbc52bfbdabd7e3ea0e864b13b578c5632f12d311ed4d82712361`.
