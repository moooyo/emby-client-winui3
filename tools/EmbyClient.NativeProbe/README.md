...
`--shared-player-control` changes only the HTTP choice relative to that shared-player control: it restores the original product `ScopedAdaptiveHttpFilter` and `ScopedMediaTransport` without activating the native HTTP construction hook. The mode is `SharedPlayerManagedHttpHlsControlNotProductAcceptance`. Per-open and post-seek booleans must confirm the real product filter exists; native HTTP control counters remain zero because that control is disabled. The same source/player checks, twenty cycles, forty server sessions, and original resource gate apply. The product's authentication, redirect, and bounded-download semantics remain in force, and the result remains an isolated control.
