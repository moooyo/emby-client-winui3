...
The latest development AOT executable SHA-256 is `B1CBF350968A64456C975932591CDD62FC08D3ED72BDD6892925CEF76F9AD7A5`. It includes collection Reset poster cleanup and excludes the optional library observer. Detailed UI checkpoints identify which artifact establishes each observation. Display and placement services are partial types so the C#/WinRT source generator supplies AOT-compatible interface metadata; their initial `CsWinRT1028` warnings have been resolved.
...
| Hosted Windows CI | **All steps passed** for `b0ed0a9` in [run 34306180821](https://github.com/moooyo/emby-client-winui3/actions/runs/34306180821) | Independent locked restore, Release build, 340 tests, AOT/SBOM output, unsigned MSIX construction, and artifact uploads; later image-lifecycle changes need their own CI checkpoint |
| Unsigned MSIX checkpoint | **Structural verification passed** for the B1CBF350 publish; 332 payload files and 289 preserved resource keys | Byte-for-byte unpack round trip, notices and SBOM inclusion; no installed runtime claim |
...
| Large-library trial | Six pages and repeated browsing work. The c28f instrumented Reset fix leaves one bound source and zero pending requests on Home; normal B1CBF350 also displays posters after detail navigation. | Private-memory convergence remains unverified. A screenshot-only control also grew, so requested UIA traversal is not its sole explanation. Process windows are unequal and do not establish a cleanup effect size. |
