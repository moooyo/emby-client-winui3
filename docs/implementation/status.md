...
The latest development AOT executable SHA-256 is `77A90D2DD8D3AD527BFFCFF40361BEAB39FDC2A8F3C90F72E2340506BB21D9E8`. It includes collection Reset poster cleanup, keyboard timeline editing ownership, and restored Queue/Diagnostics focus; the optional library observer is excluded. Detailed UI checkpoints identify which artifact establishes each observation. Display and placement services are partial types so the C#/WinRT source generator supplies AOT-compatible interface metadata; their initial `CsWinRT1028` warnings have been resolved.
...
| Hosted Windows CI | **All steps passed** for `076dbbe` in [run 34309591888](https://github.com/moooyo/emby-client-winui3/actions/runs/34309591888) | Independent locked restore, Release build, 340 tests, AOT/SBOM output, unsigned MSIX construction, and artifact uploads, including collection Reset poster cleanup |
| Unsigned MSIX checkpoint | **Structural verification passed** for the 77A90D2D publish; 332 payload files and 289 preserved resource keys | Byte-for-byte unpack round trip, notices and SBOM inclusion; no installed runtime claim |
