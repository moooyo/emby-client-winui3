...
| Automated checks | Unified Release run: **253 passed** across API 47, media transport 65, Windows platform 33, and playback 108. | Most HTTP/engine checks use test adapters; these are not a broad decoder or server certification. |
...
| Protected account persistence | 33 platform tests exercise actual Windows user-scoped protection, atomic settings, and connection/logout races. | No roaming or portable credential-store claim. |
...
| Real-server HLS lifecycle | 20 functional cycles pass: 0/17-second starts, new-session seeks to 45 seconds, restored pause, 40 encoding cleanups, and no diagnostics. | **Resource gate failed: +52 handles exceeds 32**; private-memory growth is approximately 4.51 MB. This remains unresolved. |
