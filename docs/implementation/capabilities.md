...
| Direct-path resources | The default shared-owner 20-cycle relay/SMTC run passes the unchanged gate: **+17 handles** against limit **32**, **+2,166,784 private bytes**. | Separate from broad codec or long-duration behavior. |
...
| Integrated UI and fullscreen | C6E8E17B shows actual shared-owner video, paused HLS/audio/version changes, editable queue, local diagnostics, and close while playing. Earlier runs also show burned SRT and fullscreen entry/Escape exit. | Narrator, device output, and native external captions retain separate gates. |
