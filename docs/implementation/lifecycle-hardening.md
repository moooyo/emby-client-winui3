...
The [completed local batch](verification/code-batch-20260910.json) passes 440 tests (47 API, 125 transport, 132 platform, 136 playback), Release compilation, Native AOT, SBOM generation/readback, and unsigned MSIX structural checks. Its candidate is `779B23ED8D28B845860AB72ABE1764D60FCDA7D9B2F6A089EFCFEA912BCBA93C`. No player or desktop action was run. The first batch failed the new pre-cancelled logout case: local persistence succeeded, but remote revocation lacked an explicit pre-cancellation check. The service now checks before reaching the transport. That failed receipt remains preserved rather than being relabeled as passed.
