...
### Session-triggered 257B library and Home observation

The user authorized actual desktop testing of schema 3 candidate `257B0F462E2B0D45C34EB6632E2C29EDF184310047905FC48BC069D5E3212CBE`, PID 20760. Its [receipt and immutable evidence](verification/ui-257b-20260910/summary.json) retain 154 samples over 766.276 seconds, eleven original screenshots, fixture counters, and the source interpretation. All 69 build-manifest source hashes still matched after desktop work. This is the optional observation build, not normal candidate 0E84F458 or earlier 779B23ED. No video was opened, no GC was forced, and no local build or other native probe ran during the measurements.

The initial Home sample was frozen before desktop navigation. Excluding its first 30 seconds leaves 55 rows over 270.396 seconds: private bytes increase 1.1641 MiB, managed bytes 1.0837 MiB, and approximate cumulative allocation 1.0661 MiB. There is no GC, and initial poster work has already stopped. This baseline has no contemporaneous screenshot; the later first desktop screenshot independently confirms connected Home and two visible posters.

Five manual Load more actions after the initial page requested offsets 0 through 240. Reaching that loaded end triggered one additional incremental page at offset 288, making the actual count **336 of 5,000**. Screenshots confirm the sampled posters around items 206 and 288, three returns to the top, and two middle revisits around items 181-190. The list portion of the numeric trace spans 160.391 seconds; private bytes increase from 138.2969 to 249.1445 MiB during that interval. The sequence is shorter and different from the earlier 779B experiment and is not an A/B comparison or a complete traversal of 5,000 records.

Home returns at elapsed 525.948 seconds. Excluding the next 30 seconds leaves **43 rows over 210.273 seconds**: private bytes decrease from **253.2734 to 252.0195 MiB**, working set decreases 1.2109 MiB, and handles fall from **1,385 to 1,259**. ManagedBytes increases 0.6888 MiB and approximate cumulative allocation increases 0.6410 MiB, without another GC. CPU increases 734 ms, about 0.3491% of one core's capacity. QueryCount stays 28, image requests stay 666, and both endpoint fixture snapshots report zero active images.

The settled Home samples have two non-null poster sources, zero pending poster requests or active decodes, and unchanged decode totals of 440 started, 376 completed, 64 canceled, and zero failed. Presentation-clock ticks and position-update calls remain zero throughout the complete no-video prefix. The 46 subscribed images still have container ancestors, but only two have entries in the application's current container map. These fields are subsets of the subscription table, not an independent native/viewport census. The [source review](verification/ui-257b-20260910/source-interpretation.md) explains the strong subscription keys, pooled controls, and cleanup semantics.

The weak bitmap cohort ends with 376 recorded targets, of which 126 still resolve. Earlier natural collections reduce some weak targets, but the surviving targets cannot be assigned to native, XAML, application, or transient owners from these counters. Last-GC fields describe the last collection, not the current live heap. The finite Home decline therefore narrows the earlier sustained-growth observation without establishing a leak fix, long-term convergence, or release acceptance.

Desktop release was recorded at 00:04:32 UTC after saving the final Home screenshot, and the Computer Use JavaScript context was immediately reset. The rest of this prefix was read passively while desktop control remained released; there were no further automated screenshots or input. User activity after release was not observed. The app remained open, and its original bounded logger may continue beyond the frozen prefix. No observation outside the frozen file is implied by this report.

The same logger subsequently reached its thirty-minute bound. An exclusive file read confirmed its writer was closed while PID 20760 remained alive. The [completed observation](verification/ui-257b-20260910/completed-session-observation.json) preserves 360 rows, 515,161 bytes, and elapsed 1,797.954 seconds; its first 154 rows match the earlier frozen prefix. Logger completion is distinct from application exit.

The [completed Home analysis](verification/ui-257b-20260910/completed-session-analysis.json) extends the same settled interval to 249 rows over **1,241.951 seconds**. Private bytes change **-0.7266 MiB**, working set **-0.7500 MiB**, managed bytes **+3.2032 MiB**, and approximate cumulative allocation **+3.1351 MiB**. Handles change from 1,385 to 1,258; CPU increases 2.953 seconds, about 0.2378% of one core's capacity. No further GC occurs, and poster, decode, presentation-clock, and weak-target counters stay unchanged. The final fixture snapshot still has QueryCount 28, image requests 666, and zero active images.

This longer finite Home interval does not show sustained private-byte growth, but the surviving weak bitmap targets remain unattributed and the catalog workload is unchanged. It neither proves native resource release nor completes the broader library/performance gate. The synchronous sampler's partial allocation estimate must not be treated as its entire overhead. No further polling of this completed writer is needed.

### BA6A pre-sign-in observation

The user authorized another desktop session after the startup-initialization repair. The BA6AFB8D instrumented client, PID 15620, launched against the existing synthetic account and reached its manual saved-sign-in page. The [frozen 87-row prefix](verification/ui-ba6a-presignin-20260910/pre-sign-in-snapshot.jsonl), [analysis](verification/ui-ba6a-presignin-20260910/pre-sign-in-analysis.json), and [initial screenshot](verification/ui-ba6a-presignin-20260910/01-saved-sign-in-ready.jpg) cover a period when that manual handoff was still pending. The live logger remains a separate file; this prefix does not mark the application or library test complete.

Excluding the first approximately 30 seconds leaves 81 rows spanning 400.613 seconds, from 2026-09-09 21:20:47.852 to 21:27:28.465 UTC. ManagedBytes rises **7.1016 MiB**, approximate cumulative managed allocation **7.0673 MiB**, private bytes **6.7969 MiB**, and working set **7.8125 MiB**. CPU time rises 13.719 seconds, averaging 3.4245% of one core's capacity; handles change from 1,034 to 1,037. Completed synchronous observation calls account for 999,728 bytes of measured current-thread allocation, an incomplete estimate of the observer's cost.

All rows report zero library items, poster/decode/weak-target activity, presentation-clock ticks, and position-update calls. Gen 0/1/2 collection counts and LastGcIndex are also zero. This recorded allocation growth therefore does not require poster decoding or the playback timer. It does not prove a leak or establish the cause of the older 779B Home trend: GetTotalMemory(false) includes uncollected garbage, no GC has occurred, and this is an instrumented sign-in page with different controls and work. The sampled overhead estimate cannot be subtracted as the observer's entire cost.

Further attribution must include window/sign-in controls and observer activity. The BA6A emitted link response references `eventpipe-disabled.lib`, and its ILC feature sets `System.Diagnostics.Tracing.EventSource.IsSupported=false`; no diagnostic attachment was attempted. Microsoft's [Native AOT diagnostics documentation](https://learn.microsoft.com/en-us/dotnet/core/deploying/native-aot/diagnostics) describes EventPipe as optional through EventSourceSupport. Enabling that support would require a separately identified build, not an assumption that the current executable can supply allocation traces. No code or build was changed during this running observation session.

The [later 206-row prefix](verification/ui-ba6a-presignin-20260910/pre-sign-in-first-gc-snapshot.jsonl) preserves the original 87 rows and adds the first natural Gen 0 collection. The [independently checked analysis](verification/ui-ba6a-presignin-20260910/pre-sign-in-first-gc-analysis.json) locates the sampled transition between rows 125 and 126, a 5.006-second interval. ManagedBytes falls **13.8369 MiB (90.978%)**, while private bytes rise 1.1875 MiB. The first available latest-GC snapshot reports a 1.3072 MiB heap and 16.4570 MiB committed; these fields are not continuous live-heap readings.

The 81 samples beginning with that first post-GC sample span another 400.662 seconds. ManagedBytes rises 6.4705 MiB, while private bytes increase only 0.7305 MiB. The final 60 samples, spanning 295.490 seconds, all read **119,894,016 private bytes**. Library/poster/playback activity fields remain zero. The earlier pre-GC rise therefore includes substantial reclaimable allocation and must not be described as persistent live-object retention. This is one natural collection and a finite sampled tail on the instrumented sign-in page; it neither proves continuous or long-term stability nor completes the 779B library/Home investigation.

Both later manual sign-in handoffs arrived after their BA6A collector had expired; a visible connected library did not provide new numeric observation. The [Schema 3 repair](verification/observation-session-triggered-20260910.json) starts the collector only at the first authenticated library session setup. Its first row, not process creation, is the new baseline; process-wide allocation/GC counters can already contain login-wait history. Ordinary Release, enabled AOT, and compiled start-callsite inspection pass. No new library-memory result is implied by those build checks.
