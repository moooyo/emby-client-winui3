...
The current work mode was requested on 2026-09-10: finish the code changes first, then validate the resulting candidate together. Desktop control is paused. A stopped computer-use session, a running application, or an earlier desktop authorization does not schedule another interruption.

The [consolidated command-line batch](verification/code-batch-20260910.json) is now complete: 440 tests, Release build, Native AOT, and unsigned package checks passed. The unlaunched candidate is `779B23ED8D28B845860AB72ABE1764D60FCDA7D9B2F6A089EFCFEA912BCBA93C` in `artifacts/verification/20260910-code-batch-2/publish`. The interactive batch below remains pending.
...
The current synthetic fixture has no media-opening delay. A fast opening is not a reason to repeatedly launch and close windows hoping to catch it; record that case as inconclusive unless the actual Opening interval can be established.
