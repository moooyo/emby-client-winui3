...
The Windows adapter has a separate [progressive HTTP receive path](../implementation/progressive-streaming.md) for this timeline. It accepts a successful response without a declared length and preserves an upstream 200 when Emby ignores Range. A protocol-only control against Emby 4.9.5.0 observed chunked fragmented MP4 at 0/41-second starts. Only the initial 64 KiB was read; native decoding, the actual trim origin, and end-of-stream behavior remain separate acceptance checks. The default application profile still requests HLS.
